...
def main() -> None:
    parser = argparse.ArgumentParser(description="United Rugby Championship predictions")
    try:
    ...
    l4446 = cast(pd.DataFrame, upcoming[upcoming["league_id"] == 4446].copy())
    ...
    try:
        ...
        print(l4446.reindex(columns=preview_cols))
    except Exception:
        pass

    hist = cast(
        pd.DataFrame, df[(df["league_id"] == 4446) & df["home_win"].notna()].copy()
    )
    ...
    if len(hist) == 0 or len(present_cols) == 0:
        print("Insufficient historical data to train models for league 4446.")
        return

    # Team-level baselines
    home_wr_series = (
    ...
    try:
        ...
        upc_ids: set[int] = _to_id_set(_col_or_empty(l4446, 'home_team_id')) | _to_id_set(_col_or_empty(l4446, 'away_team_id'))
        for tid in upc_ids:
            if tid in hist_ids:
                team_alias[tid] = tid
                continue
    ...
    # Ensemble OOF probabilities from calibrated LR and GBDT
    ...
    print("\n=== United Rugby Championship Predictions (Neutral Mode) ===")
    print(f"League: 4446 | Fixtures: {len(l4446)}")
    ...
    if args.export_csv and season_summary is not None and hist_pred_df is not None and calib_df is not None:
        season_summary.to_csv("artifacts/urc_oof_per_season.csv", index=True)
        hist_pred_df.to_csv("artifacts/urc_oof_predictions.csv", index=False)
        calib_df.to_csv("artifacts/urc_calibration_curve.csv", index=False)

    team_names: dict[int, str] = {}

    # OOF listing (regularized)
    try:
        ...
        print("\nURC historical (OOF predictions and score accuracy):")
    ...
    try:
        ...
        mapped_home = l4446["home_team_id"].apply(lambda tid: home_elo_map.get(team_alias.get(safe_to_int(tid, default=-1), safe_to_int(tid, default=-1)), np.nan) if not is_missing(tid) else np.nan)
        mapped_away = l4446["away_team_id"].apply(lambda tid: away_elo_map.get(team_alias.get(safe_to_int(tid, default=-1), safe_to_int(tid, default=-1)), np.nan) if not is_missing(tid) else np.nan)
        l4446["elo_home_pre"] = cast(pd.Series, l4446["elo_home_pre"]).combine_first(cast(pd.Series, mapped_home)).fillna(1500.0)
        l4446["elo_away_pre"] = cast(pd.Series, l4446["elo_away_pre"]).combine_first(cast(pd.Series, mapped_away)).fillna(1500.0)
    except Exception:
        pass
