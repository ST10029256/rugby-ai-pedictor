...
def main() -> None:
    parser = argparse.ArgumentParser(description="Rugby World Cup predictions")
    try:
    ...
    l_league = cast(pd.DataFrame, upcoming[upcoming["league_id"] == 4574].copy())
    ...
    hist = cast(pd.DataFrame, df[(df["league_id"] == 4574) & df["home_win"].notna()].copy())
    ...
    if len(hist) == 0 or len(present_cols) == 0:
        print("Insufficient historical data to train models for league 4574.")
        return
    ...
    hist = hist.copy()
    # Use dict-based mapping for stricter typing
    ...
    # Summary metrics (ensemble OOF: calibrated LR + GBDT)
    ...
    cv_acc = float(np.mean((prob_preds >= 0.5).astype(int) == y_hist))
    brier = brier_score_loss(y_hist, prob_preds)
    ll = log_loss(y_hist, prob_preds)

    print("\n=== Rugby World Cup Predictions (Neutral Mode) ===")
    print(f"League: 4574 | Fixtures: {len(l_league)}")
    ...
    try:
        ...
        print("\nRugby World Cup historical (OOF predictions and score accuracy):")
        for i in range(len(hist)):
            ...
            print(f"{row_h['date_event']} | {home_nm} vs {away_nm} | actual {int(row_h['home_score'])}-{int(row_h['away_score'])} | pred {ph:.1f}-{pa:.1f} | pred_winner={'Home' if prob_preds[i]>=0.5 else 'Away'} (prob {prob_preds[i]:.1%}) | correct={bool((prob_preds[i]>=0.5)==(y_hist[i]==1))} | abs_err=({abs(ph-y_home[i]):.1f},{abs(pa-y_away[i]):.1f})")
    except Exception:
        pass
    ...
    # Per-team residual std maps for intervals
    ...
    if len(l_league) > 0:
        ...
        # Win rates mapping (dict-based apply)
        ...
        upc["home_wr_home"] = upc["home_wr_home"].where(
            ...
            upc["home_alias"].apply(lambda tid: _home_wr_map.get(safe_to_int(tid, -1), float("nan")))
        )
        upc["away_wr_away"] = upc["away_wr_away"].where(
            upc["away_wr_away"].notna(),
            upc["away_alias"].apply(lambda tid: _away_wr_map.get(safe_to_int(tid, -1), float("nan")))
        )
