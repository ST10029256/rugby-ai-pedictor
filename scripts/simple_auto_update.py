#!/usr/bin/env python3
"""
Simplified auto-update script for upcoming rugby games.
This version focuses on the most reliable data sources and provides clear instructions.
"""
...
# League mappings - focus on the most reliable sources
LEAGUE_MAPPINGS = {
    # Internal DB ID -> (API Provider, API League ID, League Name, Notes)
    4986: ("apisports", 85, "Rugby Championship", "Requires API-Sports API key"),
    4574: ("thesportsdb", 4574, "Rugby World Cup", "Works with free TheSportsDB key (123)"),
    # Note: URC and Currie Cup may need premium API keys for reliable data
}
...
def get_upcoming_games_thesportsdb(client: TheSportsDBClient, league_id: int, days_ahead: int = 30) -> List[Dict[str, Any]]:
    """Get upcoming games from TheSportsDB for a specific league ID."""
    try:
        ...
        logger.info(f"Found {len(upcoming_games)} upcoming games for TheSportsDB league {league_id}")
        return upcoming_games
...
def main():
    parser = argparse.ArgumentParser(description="Simplified auto-update for upcoming rugby games")
    ...
    thesportsdb_api_key = os.getenv("THESPORTSDB_API_KEY", "123")  # Default to free key
    ...
    for league_id, (provider, api_id, name, notes) in LEAGUE_MAPPINGS.items():
        if league_id in leagues_to_update:
            logger.info(f"  {name} ({league_id}): {notes}")
    logger.info("=" * 60)
    ...
    for league_id in leagues_to_update:
        ...
        provider, api_league_id, league_name, notes = LEAGUE_MAPPINGS[league_id]
        ...
        try:
            if provider == "apisports":
                ...
                if not apisports_api_key:
                    ...
                    logger.info("  To enable: Set APISPORTS_API_KEY environment variable")
                    continue
                ...
                games = get_upcoming_games_thesportsdb(client, api_league_id, args.days_ahead)
    ...
    logger.info("1. For API-Sports (Rugby Championship):")
    logger.info("   - Get API key from: https://api-sports.io/")
    logger.info("   - Set environment variable: APISPORTS_API_KEY=your_key")
    logger.info("2. For TheSportsDB (Rugby World Cup):")
    logger.info("   - Free key '123' works for basic data")
    logger.info("   - Set environment variable: THESPORTSDB_API_KEY=your_key")
    logger.info("3. Run this script daily via cron or GitHub Actions")
    logger.info("=" * 60)
